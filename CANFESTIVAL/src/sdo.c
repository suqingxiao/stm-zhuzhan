...
UNS8 getSDOlineOnUse (CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS8 *line)
{
	...
	for (i = 0 ; i < SDO_MAX_SIMULTANEOUS_TRANSFERS ; i++)
	{
//		if ( (d->transfers[i].state != 0x0) &&                  //0x01 0x0 SDO_RESET
//				(d->transfers[i].state != SDO_ABORTED_INTERNAL) &&  //0x1 0x85
//				(d->transfers[i].CliServNbr == CliServNbr) &&//0x7f 0x00
//				(d->transfers[i].whoami == whoami) )
		if(d->transfers[i].state != 0x0)
		{
			if(d->transfers[i].state != SDO_ABORTED_INTERNAL)
			{
				if(d->transfers[i].CliServNbr == CliServNbr)
				{
					if(d->transfers[i].whoami == whoami)
					{
							if (line) *line = i;
							return 0;
					}
...
UNS8 sendSDO (CO_Data* d, UNS8 whoami, UNS8 CliServNbr, UNS8 *pData)
{
	...
	if ( whoami == SDO_SERVER )	{
		...
		/* Get the client->server cobid.*/
		offset = d->firstIndex->SDO_CLT;//4
		if ((offset == 0) || ((offset+CliServNbr) > d->lastIndex->SDO_CLT)) {
			MSG_ERR(0x1A42, "SendSDO : SDO client not found", 0);
			return 0xFF;
		}
		m.cob_id = UNS16_LE( (UNS16) *((UNS32*) d->objdict[offset+CliServNbr].pSubindex[1].pObject) );//600+1/2/3/7f
...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	/* Am-I a server ? */
	offset = d->firstIndex->SDO_SVR;  //4
	lastIndex = d->lastIndex->SDO_SVR;//4
	j = 0;
	...
	if (whoami == SDO_UNKNOWN)   //从站不应进来 客户端是主站
	{
		...
		if(offset) while (offset <= lastIndex) 
		{
			...
			/* Looking for the cobid received. */
			pCobId = (UNS32*) d->objdict[offset].pSubindex[2].pObject;//masterobject_objdict->indexetable masterobject_objdict[4].pSubindex[2]=
			if (*pCobId == UNS16_LE(m->cob_id) ) 
			{
				whoami = SDO_CLIENT;
